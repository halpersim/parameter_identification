function [success,q,all_solutions] = inverse_kinematics_analytic(x_d,param,find_minimum_norm)
  ...
  d__bs=param.d1+param.d2;
  d__se=param.d3+param.d4;
  d__ew=param.d5+param.d6;
  d__wt=param.d7+param.d8;
  l__bs=[0;0;d__bs];
  l__se=[0;-d__se;0];
  l__ew=[0;0;d__ew];
  l__wt=[0;0;d__wt];
  
  %Transform the desired point x_d from world coordinates to robot base coordinates.
  R_w = [cos(param.yaw)*cos(param.pitch), -sin(param.yaw)*cos(param.roll) + cos(param.yaw)*sin(param.pitch)*sin(param.roll), sin(param.yaw)*sin(param.roll) + cos(param.yaw)*sin(param.pitch)*cos(param.roll); sin(param.yaw)*cos(param.pitch), cos(param.yaw)*cos(param.roll) + sin(param.yaw)*sin(param.pitch)*sin(param.roll), -cos(param.yaw)*sin(param.roll) + sin(param.yaw)*sin(param.pitch)*cos(param.roll); -sin(param.pitch), cos(param.pitch)*sin(param.roll), cos(param.pitch)*cos(param.roll)];
  p_w = [param.dw0x; param.dw0y; param.dw0z];
  H_w0=[R_w,p_w;0,0,0,1];
  H_7=H_w0\x_d;
